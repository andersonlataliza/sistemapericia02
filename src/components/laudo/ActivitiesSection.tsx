...
export default function ActivitiesSection({ value, onChange }: ActivitiesSectionProps) {
      ...
      if (isSpeechRecognitionAvailable()) {
        ...
        recognitionHandleRef.current = startSpeechRecognition({
          ...
          onResult: (text) => {
            ...
            // Anexar somente resultados finais ao campo principal, sem apagar conteúdo existente
          ...
          onError: (err) => {
            ...
            // Fallback: tentar gravação via MediaRecorder
            try {
  ...
  const exportTranscriptionAppend = () => {
    ...
    onChange(newValue);
    toast({ title: "Exportado (anexado)", description: "Transcrição adicionada à descrição detalhada." });
  };
  ...
  const exportTranscriptionReplace = () => {
    ...
    onChange(text);
    toast({ title: "Exportado (substituído)", description: "Descrição detalhada substituída pela transcrição." });
  };
  ...
  const clearTranscription = () => {
    ...
    try {
      ...
      // @ts-ignore interim buffer may exist
  ...
  const proofreadDescription = async () => {
    ...
    if (!text) {
      toast({ title: "Nada para revisar", description: "Digite a descrição detalhada antes de revisar.", variant: "destructive" });
      return;
    }
    setIsProcessing(true);
  ...
  return (
    <Card className="shadow-card">
      <CardHeader>
        <CardTitle>12. Atividades da(o) Reclamante</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Grave um áudio descrevendo as atividades. O sistema irá transcrever automaticamente o que foi dito.</p>
        </div>
      </CardContent>
    </Card>
  );
